...
def test_claude_probe_reports_invalid_model(client, monkeypatch) -> None:
    monkeypatch.setattr(
        "app.services.llm_probe_service.get_settings",
        lambda: SimpleNamespace(
            claude_api_key="env-key",
...
def test_claude_probe_reports_network_failures_cleanly(client, monkeypatch) -> None:
    monkeypatch.setattr(
        "app.services.llm_probe_service.get_settings",
        lambda: SimpleNamespace(
            claude_api_key="env-key",
    ...
    class FakeHttpxClient:
        ...
        def post(self, *args, **kwargs):
            raise httpx.ConnectError("network stack unavailable")
